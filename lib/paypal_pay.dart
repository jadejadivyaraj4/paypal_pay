...
import 'package:paypal_pay/src/PaypalServices.dart';
import 'package:paypal_pay/src/screens/complete_payment.dart';
...
class UsePaypalState extends State<UsePaypal> {
  ...
  void initState() {
    ...
    setState(() {
      navUrl = 'https://api.sandbox.paypal.com';
    });
      ...
      onWillPop: () async {
        if (pressed < 1) {
         ...
         // final snackBar = SnackBar(content: Text('Press back ${3 - pressed} more times to cancel transaction'));
          final snackBar = SnackBar(content: const Text('Press again to cancel transaction'));
                              ...
                              navigationDelegate: (NavigationRequest request) async {
                                ...
                                if (request.url.contains(widget.returnURL)) {
                                  Navigator.pushReplacement(
                                    context,
                                    MaterialPageRoute(builder: (context) => CompletePayment(url: request.url, services: services, executeUrl: executeUrl, accessToken: accessToken, onSuccess: widget.onSuccess, onCancel: widget.onCancel, onError: widget.onError)),
                                  );
                                }
